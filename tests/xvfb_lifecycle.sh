...
tmp_dir=$(mktemp -d)
xvfb_pid= wm_pid= transient_pid= override_pid= ordinary_pid= dock_pid= border_pid=

cleanup() {
    for pid in "$border_pid" "$transient_pid" "$override_pid" "$ordinary_pid" "$dock_pid" "$wm_pid" "$xvfb_pid"; do
