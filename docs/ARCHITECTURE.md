...
stay managed while Box2430 unmaps them. MONOCLE also keeps inactive tabs
unmapped while preserving their full client state. Returning to that workspace
or activating that tab remaps the same client without re-running the management
pipeline.
...
Workspace hiding and inactive MONOCLE tabs use real `XUnmapWindow` calls.
Applications may also unmap a top-level window to withdraw it from WM
management.
...
MONOCLE intentionally couples tab activation with focus and raising. The active
tab is the workspace's mapped ordinary client; inactive tabs remain managed but
unmapped. Activating another tab maps it before focus is projected, raises it,
then unmaps the previous tab. Focus and stacking remain separate semantics in
FREE mode.
...
* the active/focus-target tab is mapped and presented in the monitor's MONOCLE
  content rectangle;
* inactive tabs remain managed but unmapped;
* tab activation maps the incoming tab first, focuses and raises it, then hides
  the previous tab;
...
Only mapped clients on active workspaces participate in visible presentation.
FREE maps every client on the active workspace; MONOCLE maps only its active
tab. Workspace switching briefly overlaps the incoming presentation and the
outgoing mapped presentation, but stacking enforcement follows the newly active
workspace so outgoing clients are not raised back over the incoming workspace.
...
4. map the incoming presentation (all FREE clients, or only the MONOCLE active
   tab);
5. unmap the outgoing presentation with `ignored_unmaps` bookkeeping;
6. focus the chosen incoming target;
